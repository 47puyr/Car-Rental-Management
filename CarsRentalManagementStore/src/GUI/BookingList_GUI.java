...
public class BookingList_GUI extends javax.swing.JFrame {
    ...
    private void exportPDF(File file, JTable table) throws DocumentException, IOException {
        ...
        // Add car icon
        Image carIcon = Image.getInstance("D:\\Java\\DoAnJava\\CarsRentalManagementStore\\src\\images\\car_bill.png");
        carIcon.scaleToFit(50, 50);
        carIcon.setAlignment(Element.ALIGN_CENTER);
        document.add(carIcon);
    ...
    private void jButton_selectbookingActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton_selectbookingActionPerformed
        ...
        try {
            ...
            String id = jTable_bookinglist.getValueAt(index, 0).toString();
            ...
            Booking_Edit_Remove_GUI.displayBooking(id, car_id, customer_id, pickup_city, pickup_address, pickup_date, pickup_time, dropoff_city, dropoff_address, dropoff_date, dropoff_time, total_price);
    ...
    private void jButton_printTXTActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton_printTXTActionPerformed
        ...
        try {
            File file = new File("D:\\Java\\DoAnJava\\CarsRentalManagementStore\\booking.txt"); 
            if(!file.exists())
            {
                file.createNewFile();
            }
    ...
    private void jButton_printPDFActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton_printPDFActionPerformed
        try {
            File fileToSave = new File("D:\\Java\\DoAnJava\\CarsRentalManagementStore\\booking_list.pdf");
    ...
    private void jButton_importexcelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton_importexcelActionPerformed
        ...
        String defaultCurrentDirectoryPath = "D:\\Java\\DoAnJava\\CarsRentalManagementStore";
        ...
        if (excelChooser == JFileChooser.APPROVE_OPTION) {
            try {
                ...
                DefaultTableModel model = (DefaultTableModel) jTable_bookinglist.getModel();
                model.setRowCount(0);  

                Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/java_car_rental", "root", "");
                conn.setAutoCommit(false);  

                String insertSQL = "INSERT INTO `reservation`(`id`, `car_id`, `customer_id`, `pickup_city`, `pickup_address`, `pickup_date`, `pickup_time`, `dropoff_city`, `dropoff_address`, `dropoff_date`, `dropoff_time`, `total_price`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
                ...
                for (int row = 1; row <= excelSheet.getLastRowNum(); row++) { 
                        ...
                        if (cell != null) {
                            switch (cell.getCellType()) {
                                case STRING:
                                    ...
                                    pstmt.setString(col + 1, cell.getStringCellValue());
                                    break;
                                case NUMERIC:
                                    if (DateUtil.isCellDateFormatted(cell)) {
                                        rowData.add(cell.getDateCellValue());
                                        pstmt.setDate(col + 1, new java.sql.Date(cell.getDateCellValue().getTime()));
                                    } else {
                                        rowData.add(cell.getNumericCellValue());
                                        pstmt.setDouble(col + 1, cell.getNumericCellValue());
                                    }
                                    break;
                                case BOOLEAN:
                                    rowData.add(cell.getBooleanCellValue());
                                    pstmt.setBoolean(col + 1, cell.getBooleanCellValue());
                                    break;
                                default:
                                    rowData.add("");
                            ...
                            rowData.add(null); 
                            pstmt.setNull(col + 1, java.sql.Types.NULL);
                        }
                    }
                    ...
                    if(rowData.size() == 12) {
                        ...
                        JOptionPane.showMessageDialog(null, "Row " + (row+1) + " does not have enough columns. Expected 12, but got " + rowData.size());
                    }
                }
    ...
    private void jButton_exportexcelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton_exportexcelActionPerformed
        ...
        try {
            JFileChooser excelFileChooser = new JFileChooser("D:\\Java\\DoAnJava\\CarsRentalManagementStore");
